...
Android с новой палитрой собран и прошёл стартовую проверку; ниже уже показаны его реальные экраны. Обновлённая Windows-версия ещё собирается. Индикатор страниц Android теперь тоже соответствует палитре. Повторная проверка прошла, включая запуск по значку и переключение тем. [Как устроено оформление и что перенесено из концепта](docs/APPEARANCE.md).

## Как выглядит приложение

Ниже — **реальные снимки тестовых сборок**. Стартовые экраны сняты без входа в аккаунт; внутренние Windows-экраны — в тестовом аккаунте со специально созданными вымышленными каналами. Android показан с новой палитрой, Windows — в предыдущей проверенной сборке.
...
### Внутри Windows-клиента

Папка «Демо», вымышленный книжный клуб и редактор личной заметки в **предыдущем светлом оформлении**. Это снимки работающего приложения, а не макеты. Имена каналов и тексты придуманы для демонстрации; личных переписок на снимках нет.

<p><img src="docs/images/windows-demo-chat.png" width="760" alt="Работающий Windows-клиент: папка Демо и вымышленный книжный клуб, прежнее оформление"></p>
<p><img src="docs/images/windows-demo-note.png" width="760" alt="Редактор локальной заметки с вымышленным планом встречи книжного клуба"></p>

Снимок заметки подтверждает открытие редактора и ввод текста. Сохранение с повторным открытием ещё проверяется. В канале видна стандартная плашка Telegram для недавно добавленного канала. Новое оформление переписки, внутренние экраны Android, шаблоны и тёмная форма входа Windows будут добавлены после проверки.
