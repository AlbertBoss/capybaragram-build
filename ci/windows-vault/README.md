...
The preceding native proof covers the previous Registry revision. The reset-barrier and Worker changes require their own exact-source native test run. A crash in the interval between Telegram persisting a fresh login and the asynchronous host recording its new vault generation still requires an integration-level durability check before release.
