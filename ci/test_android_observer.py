...
for path in paths:
    ...
    for mode in ['always_missing','missing_then_fresh','malformed_then_fresh']:
        with tempfile.TemporaryDirectory(prefix='capy-freshness-') as directory:
            ...
            if mode=='always_missing':
                ...
                assert len(fresh_paths)==4 and not (Path(directory)/'test.xml').exists()
            else:
                tree=space['snapshot']('test')
                assert tree.find('node').get('text')=='FRESH' and len(fresh_paths)==2
            assert len(fresh_paths)==len(set(fresh_paths))
            assert '/sdcard/capy-ui.xml' not in fresh_paths
            print(path.name,mode,'PASS')
print('Six observer regression cases passed: zero-exit dump without a new file never reuses stale UI; missing/malformed hierarchy retries require fresh output.')
